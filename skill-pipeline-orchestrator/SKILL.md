...
description: Use when the user says "一键发布 skill", "走完整 pipeline", "发布这个 skill", or "skill 上线". Use when a new skill has passed review and is ready for release across one or more agent platforms.
version: 1.0.0
metadata:
  tags: [pipeline, orchestration, deployment, workflow]
  related_skills: [agent-config-reference, agent-security-guard, cross-model-verification, git-workflow-for-agents, skill-authoring-workflow, skill-review-workflow]
---
...
安装到各平台
  │
  ├── 先检查目标路径是否已存在
  │     ├── 已存在 → 询问用户：overwrite / backup / skip
  │     └── 不存在 → 继续安装
  │
...
| Stage 5 | 目标路径已存在 | 用户选择 overwrite / backup / skip 后再继续 |
...
- [ ] Stage 5: 已检查目标路径；存在时已获得 overwrite / backup / skip 决策
- [ ] Stage 5: 4 个平台全部安装成功
