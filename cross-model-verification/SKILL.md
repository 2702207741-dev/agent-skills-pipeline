...
Artifact contains secrets, credentials, production data, customer data, or private repo context?
  ...
  │     ├── Replace values with placeholders: <REDACTED_API_KEY>, <CUSTOMER_ID>, <INTERNAL_HOST>
...
```bash
grep -Ei "sk-|AKIA|AIza|xox|password|secret|private_key|token|BEGIN .*PRIVATE KEY" <artifact-file>
...
**Expected Output:** `findings classified as valid / trade-off / noise / contract-misread`
...
**Expected Output:** `severity-classified report with approve / request changes / needs discussion`
