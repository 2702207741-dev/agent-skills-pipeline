...
description: Use when the user says "审查这个skill""review skill""检查skill质量""这个skill写得怎么样". Use when a new SKILL.md is ready for pre-commit quality validation. Use when the user wants an objective quality score for an existing skill. Do NOT use during drafting stage, for reviewing non-SKILL.md code (use code-review skill), or when the user asks how to use a skill (load and explain directly).
version: 1.1.0
metadata:
...
| 多个 skill 需要批量审查 | 审查的是 .py/.js/.ts 代码而非 SKILL.md（用 code-review skill） |
